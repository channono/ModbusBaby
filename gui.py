...
class ModbusBabyGUI(QMainWindow):
    ...
    def read_register(self):
        ...
        if self.modbus_debugger:
            sent_packet, received_packet = b'', b''
            try:
                ...
                if result is not None:
                    ...
                    if register_type == 'Report Slave ID (FC11H)':
                        # For FC11H, result is a list of strings.
                        # Join for the log, but show a summary in the value input.
                        full_report = '\n'.join(result) if isinstance(result, list) else str(result)
                        summary = result[-1] if isinstance(result, list) and result else str(result) # Show last line (vendor info) as summary
                        self.value_input.setText(summary)
    ...
    def write_register(self):
        ...
        if not self.modbus_debugger or not self.modbus_debugger.client:
            self.log_output.append("错误：Modbus 客户端未连��")
            return

        register_type = self.register_type_combo.currentText()
        if register_type == 'Report Slave ID (FC11H)':
            self.log_output.append("错误：FC11H (Report Slave ID) 是一个只读功能，不支持写入。")
            return
        ...
        try:
            ...
            count = end_address - start_address + 1
            success, message, result = False, "未知错误", (b'', b'')
            ...
            if register_type == 'Coil':
                ...
                if data_type == 'BYTE':
                    ...
                    import time
                    timestamp = int(time.time()) if value.strip().lower() == 'now' else int(value.strip())
            ...
            if result and isinstance(result, tuple) and len(result) == 2:
                ...
                self.logger.warning(f"无法从写入操作中获取报文, result: {result}")
                # 即使 result 格式不正确，我们仍然尝试从 debugger 中获取最后发送的报文
                sent_packet = self.modbus_debugger.last_sent_packet
                received_packet = self.modbus_debugger.last_received_packet
        ...
        if self.modbus_debugger:
            ...
            try:
                ...
                if result is not None:
                    formatted_result = self.format_result(result, data_type)
                    self.value_input.setText(formatted_result)
                    self.log_output.append(f"轮询 {register_type} {start_address}-{end_address}: {formatted_result}")
                else:
    ...
    def update_packet_display(self):
        ...
        print(f"开始更新报文显示: 发送 {self.sent_packets}, 接收 {self.received_packets}")
    ...
    def get_system_serial_ports(self):
        self.logger.info("Attempting to get system serial ports.")
        try:
            if self.modbus_debugger is None:
                self.logger.error("ModbusDebugger instance is None, cannot get ports.")
                return []
            
            self.logger.info("Calling modbus_debugger.get_available_serial_ports...")
            ports = self.modbus_debugger.get_available_serial_ports()
            
            if not ports:
                self.logger.warning("No serial ports found by the debugger.")
            else:
                self.logger.info(f"Debugger found ports: {ports}")
            return ports
        except Exception as e:
            self.logger.error(f"An exception occurred while getting system serial ports: {str(e)}", exc_info=True)
            return []
